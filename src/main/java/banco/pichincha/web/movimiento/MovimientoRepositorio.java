...
public interface MovimientoRepositorio extends JpaRepository<Movimiento, Long> {
    ...
    Optional<Movimiento> findAllByCuentaId(Long cuentaId);
    ...
    @Query("SELECT m FROM Movimiento m WHERE m.cuenta.id = :cuentaId AND (m.fecha < :fecha OR (m.fecha = :fecha AND m.id < :movimientoId)) ORDER BY m.fecha DESC, m.id DESC")
